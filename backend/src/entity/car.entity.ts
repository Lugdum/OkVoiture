...
export class CarEntity {
    ...
    @ApiProperty({ example: "https://www.largus.fr/images/images/ORPHEA_105286_1.jpg", description: "The URL of the car image" })
    @Column({ nullable: true })
    imageUrl: string;

    @ApiProperty({ type: () => UserEntity, description: "The owner of the car (join)" })
    @ManyToOne(() => UserEntity, user => user.cars)
    owner: UserEntity;

    @ApiProperty({ type: () => [BookingEntity], description: "The booking of the car (join)" })
