    ...
    const fetchBookingsAndCarInfos = async () => {
      try {
        ...
        for (let i = 0; i < bookingsWithCarIds.length; i++) {
          const carInfoResponse = await axios.get(
            `http://localhost:4000/cars/${bookingsWithCarIds[i].car.id}`
          );
        ...
        for (let i = 0; i < bookingsWithCarIds.length; i++) {
          const bookingWithCarInfo = {
            ...bookingsWithCarIds[i],
            car: carInfoResponses[i],
          };
  ...
  const submitEditBooking = async (id: number) => {
    ...
    if (
      !(await checkAvailability(
        bookings.find((b) => b.id === id)?.car.id.toString() || ""
      ))
    ) {
              ...
              <>
                ...
                <div className="md:flex-1">
                  ...
                  <p className="text-gray-700">
                    ...
                    {booking.car.model} {booking.car.make} ({booking.car.year})
                  </p>
                  <p className="text-gray-700">
